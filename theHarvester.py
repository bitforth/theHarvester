...
def start(argv):
    ...
    for opt, arg in opts:
        if opt == '-l':
            ...
            engine = arg
            if engine not in ("google", "linkedin", "pgp", "all", "google-profiles", "bing", "bing_api", "yandex",
                              "people123", "jigsaw"):
                usage()
    ...
    if engine == "exalead":
        ...
        all_emails.extend(emails)
        print("[-] Searching in Exalead..")
        search = exalead.ExaLeadSearch(word, limit, offset)
        search.process()
        emails = search.get_emails()
        hosts = search.get_hostnames()
        all_hosts.extend(hosts)
    ...
    if virtual == "basic":
        ...
        print("==================")
        for l in host_ip:
            search = bing.BingSearch(l, limit, offset)
