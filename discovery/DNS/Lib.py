    ...
    def addname(self, name):
        ...
        for j in range(i):
            ...
            if offset + len(buf) < 0x3FFF:
                ...
                print
                'DNS.Lib.Packer.addname:',
                print
                'warning: pointer too big'
            buf = buf + (chr(n) + label)
        if pointer:
...
class DnsResult:
    ...
    def show(self):
        ...
        print
        ';; flags: %s; Ques: %d, Ans: %d, Auth: %d, Addit: %d' % (
        ...
        for q in self.questions:
            print
            ';;      %s, type = %s, class = %s' % (q['qname'], q['qtypestr'],
                                                   q['qclassstr'])
        print
        print
        ';; ANSWERS:'
        for a in self.answers:
            print
            '%-20s    %-6s  %-6s  %s' % (a['name'], `a['ttl']`, a['typename'],
                                         a['data'])
        print
        print
        ';; AUTHORITY RECORDS:'
        ...
        if self.args.has_key('elapsed'):
            print
            ';; Total query time: %d msec' % self.args['elapsed']
        print
        ';; To SERVER: %s' % (self.args['server'])
        print
        ';; WHEN: %s' % time.ctime(time.time())
