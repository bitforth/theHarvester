...
class ExaLeadSearch:
    ...
    def do_search(self):
        ...
        h.putrequest('GET', "/search/web/results/?q=%40" + self.word + "&elements_per_page=50&start_index=" + str(
            self.counter))
    ...
    def do_search_files(self, files):
        h = http.client.HTTPConnection(self.server)
        h.putrequest('GET',
                     "search/web/results/?q=" + self.word + "filetype:" + self.files + "&elements_per_page=50&start_index=" + self.counter)
        h.putheader('Host', self.hostname)
        h.putheader('User-agent', self.userAgent)
        h.endheaders()
        response = h.getresponse()
